...
// 64x8 dual-port distributed RAM for commands.  (8 slices)
ram16x8 cmem (
		.clock(memclk), .wren(memwrite),
		.wraddress(mem_addr), .data(memdin),
		.rdaddress(cmem_addr), .q(cmem_dout) );

// 64x8 dual-port distributed RAM for results.  (8 slices)
ram16x8 rmem (
		.clock(clk), .wren(rmem_write),
		.wraddress(rmem_addr), .data(rmem_din),
		.rdaddress(mem_addr), .q(memdout) );
	...
	if (reset) begin
		...
		case (state)
			s_idle: begin
				...
				state <= s_idle;
				if (start) begin
					// Address is registered so increment 
					// one cyle before needed on Altera devices
